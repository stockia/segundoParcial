...
class Cliente {
    ...
    public function InsertarCliente() {
        ...
        if (Cliente::buscarClientePorIdDocumento($this->id, $this->numeroDocumento)) {
            ...
            $consulta = $objAccesoDatos->RetornarConsulta(
                "INSERT INTO clientes (nombreApellido, tipoDocumento, numeroDocumento, email, tipoCliente, pais, ciudad, telefono, modalidadPago, estado) 
                VALUES (:nombreApellido, :tipoDocumento, :numeroDocumento, :email, :tipoCliente, :pais, :ciudad, :telefono, :modalidadPago, :estado)"
            );
            $consulta->bindValue(':estado', $estado, PDO::PARAM_STR);
            $response = "Cliente creado";
        }
        ...
        // Aquí va tu manejo de archivo y cualquier otra lógica
        // ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
            $extension = pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION);
            $nombreArchivo = $this->numeroDocumento . $this->tipoCliente . "." . $extension;
